...
def image_upload_recipe_url(recipe_id):
...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_recipe_list_limited_to_user(self):
        '''Test llist of recipes is limited to authenticated user.'''
    ...
    def test_create_recipe(self):
        ...
        curr_recipe = Recipe.objects.get(id=res.data['id'])
        for k, v in payload.items():
            self.assertEqual(getattr(curr_recipe, k), v)
        self.assertEqual(curr_recipe.user, self.user)
    ...
    def test_update_user_returns_error(self):
        ...
        new_user = create_user(email='user2@exmpale.com', password='test123')
        recipe = create_recipe(user=self.user)
    ...
    def test_create_recipe_with_existing_tags(self):
        '''Test creating a recipe with existing atg.'''
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        other_ingredient = Ingredient.objects.create(user=self.user, name='Chili')
        payload = {'ingredients': [{'name': 'Chili'}]}

        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertIn(other_ingredient, recipe.ingredients.all())
        self.assertNotIn(ingredient, recipe.ingredients.all())

    def test_clear_recipe_ingredient(self):
        '''Test clearing a recies ingredients'''
...
class ImageUploadTest(TestCase):
    ...
    def test_upload_image(self):
        ...
        url = image_upload_recipe_url(self.recipe.id)
